...
async def setup_concurrency_data(db_session):
    ...
    patients = []
    for i in range(3):
        u = User(username=f"pat{i}", password_hash=hash_password("pwd123"), role="patient")
        db_session.add(u)
        await db_session.flush()
        p = Patient(user_id=u.id, full_name=f"Pat {i}", email=f"p{i}@test.com", age=20, gender="other", phone="123")
        db_session.add(p)
